...
//		//public float rightThumbAngle;
		...
		if(sensorData.sensorInterface != null)
		{
			...
			if(bNewFrame)
			{
				if(sensorData.bodyIndexBuffer != null)
				{
					...
					float[] buffer = new float[sensorData.bodyIndexImage.Length];
					for (int i = 0; i < sensorData.bodyIndexImage.Length; i++)
					{
						buffer[i] = (float)sensorData.bodyIndexImage[i];
					}
	...
	public static bool CopyFile(string targetFilePath, string sourceFilePath, ref bool bOneCopied, ref bool bAllCopied)
	{
		...
		{
			Debug.Log("Copying " + sourceFile.Name + " library...");
